...
function jsonToGo(json, typename, options) {
            ...
            if (Array.isArray(scope)) {
                ...
                if (sliceType === "struct") {
                    ...
                    for (var j = 0; j < scopeLength; j++) {
                        var keys = Object.keys(scope[j])
                        for (var k in keys) {
                            if (keys.hasOwnProperty(k)) {
                                var keyname = keys[k];
                                if (!(keyname in allFields)) {
                                    allFields[keyname] = {
                                        value: scope[j][keyname],
                                        count: 0
                                    }
                                }
                    ...
                    var allKey = Object.keys(allFields), struct = {}, omitempty = {};
                    for (var l in allKey) {
                        if (allKey.hasOwnProperty(l)) {
                            var keyName = allKey[l], elem = allFields[keyName];
    ...
    function camelToSnake(str) {
        ...
        for (var i in str) {
            ...
            var decimal = str[i].charCodeAt(0);
